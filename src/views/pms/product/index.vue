<template>
  ...
  <div class="app-container">
      ...
      <div style="margin-top: 15px">
        ...
        >
          <el-form-item label="输入搜索：">
            <el-input
              style="width: 123px"
              v-model="goodsSearchParams.goodsName"
              placeholder="商品名称"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item label="上架状态：">
            <el-select
              v-model="goodsSearchParams.marketEnable"
              placeholder="全部"
              clearable
            >
              <el-option
          ...
          <el-form-item label="推荐状态：">
            <el-select
              v-model="goodsSearchParams.recommend"
              placeholder="全部"
              clearable
            >
              <el-option
          ...
          <el-form-item label="审核状态：">
            <el-select
              v-model="goodsSearchParams.authFlag"
              placeholder="全部"
              clearable
            >
              <el-option
      ...
      >
        ...
        <el-table-column label="商品图片" width="120" align="center">
          <template slot-scope="scope"
            ><img style="height: 80px" :src="scope.row.original"
          /></template>
        </el-table-column>
        ...
        <el-table-column label="箱规(瓶/箱)" width="100" align="center">
          <template slot-scope="scope">{{ scope.row.goodsUnit }}</template>
        </el-table-column>
        <el-table-column label="生产信息" width="200" align="center">
          <template slot-scope="scope">
            <div
              v-if="findParamsGroup(scope.row.params, '生产信息')"
              v-for="item in findParamsGroup(scope.row.params, '生产信息')
        ...
        <el-table-column label="度数容量" width="100" align="center">
          <template slot-scope="scope">
            <div
              v-if="findParamsGroup(scope.row.params, '度数容量')"
              v-for="item in findParamsGroup(scope.row.params, '度数容量')
                .goodsParamsItemDTOList"
              :key="item.paramName"
            >
              {{ item.paramName + ":" + item.paramValue }}
        ...
        <el-table-column label="等级" width="100" align="center">
          <template slot-scope="scope">
            <div
              v-if="findParamsGroup(scope.row.params, '等级')"
              v-for="item in findParamsGroup(scope.row.params, '等级')
                .goodsParamsItemDTOList"
              :key="item.paramName"
            >
              {{ item.paramName + ":" + item.paramValue }}
    ...
    <div class="batch-operate-container">
      <el-select size="mini" v-model="operateType" placeholder="批量操作">
        <el-option
...
import { putManagerGoodsGoodsDelete } from "@/apis/controller";
...
import {
  ...
  putManagerGoodsGoodsByGoodsIdUp,
} from "@/apis/controller";
import { putManagerGoodsGoodsByGoodsIdUnder } from "@/apis/controller/GuanLiDuan,ShangPinGuanLiJieKou/putManagerGoodsGoodsByGoodsIdUnder";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getManagerGoodsGoodsList({
        goodsSearchParams: this.goodsSearchParams,
      }).then((response) => {
    ...
    findParamsGroup(params, groupName) {
      return params.find((group) => group.groupName === groupName);
    },
    getBrandList() {
    ...
    handleUpdateProduct(index, row) {
      this.$router.push({ name: "updateProduct", query: { id: row.id } });
    },
    ...
    updateDeleteStatus(deleteStatus, ids) {
      ...
      // 批量删除
      putManagerGoodsGoodsDelete(params).then((res) => {
    ...
    handleCategoryChange(val) {
      this.goodsSearchParams.categoryPath =
        this.goodsSearchParams.categoryId + "," + "";
    },
  },
};
</script>
