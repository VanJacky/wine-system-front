      ...
      <div style="margin-top: 15px">
        ...
        >
          ...
          <el-form-item label="店铺状态">
            <el-select
              v-model="searchForm.storeDisable"
              clearable
              style="width: 200px"
            >
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <div style="float: right; display: flex; gap: 20px">
        <download-excel
          ...
          name="商家名单.xls"
        >
          <el-button type="primary" size="mini" class="export">
            导出商家
          </el-button>
        </download-excel>
    ...
    <div class="table-container">
      <el-table :data="data" style="width: 100%" v-loading="loading" border>
        ...
        <el-table-column label="类型" align="left" width="130">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.type === 'MIDDLEMAN'" type="success"
              >中间商</el-tag
            >
            <el-tag v-if="scope.row.type === 'MANUFACTURERS'">厂家</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="店铺状态" align="left" width="130">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.storeDisable === 'OPEN'" type="success"
              >开启中</el-tag
            >
            <el-tag v-if="scope.row.storeDisable === 'APPLY'">申请中</el-tag>
            <el-tag v-if="scope.row.storeDisable === 'APPLYING'" type="warning"
              >审核中</el-tag
            >
            <el-tag v-if="scope.row.storeDisable === 'CLOSED'" type="danger"
              >已关闭</el-tag
            >
            <el-tag v-if="scope.row.storeDisable === 'REFUSED'" type="danger"
              >审核拒绝</el-tag
            >
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="300" fixed="right">
          <template slot-scope="scope">
            ...
            >
            <el-button
              v-if="scope.row.storeDisable === 'OPEN'"
              size="mini"
              type="danger"
            ...
            >
            <el-button
              v-else-if="scope.row.storeDisable === 'CLOSED'"
              size="mini"
              type="success"
            ...
            >
            <el-button
              v-else-if="scope.row.storeDisable === 'APPLYING'"
              size="mini"
...
import {
  getManagerOrderOrder,
  putManagerStoreStoreEnableById,
} from "@/apis/controller";
import { putManagerStoreStoreDisableById } from "@/apis/controller/GuanLiDuan,DianPuGuanLiJieKou/putManagerStoreStoreDisableById";
import { putManagerStoreStoreAuditByIdByPassed } from "@/apis/controller/GuanLiDuan,DianPuGuanLiJieKou/putManagerStoreStoreAuditByIdByPassed";
  ...
  methods: {
    ...
    audit(v) {
      this.$confirm("您确认要审核通过店铺 " + v.storeName + " ?", "审核店铺", {
        ...
        putManagerStoreStoreAuditByIdByPassed({ id: v.id, passed: 0 }).then(
          (res) => {
            if (res.data.success) {
              this.$message.success("操作成功");
              this.getDataList();
